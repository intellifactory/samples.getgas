...
(function()
{
 var Global=this,Runtime=this.IntelliFactory.Runtime,WebSharper,Concurrency,websharper,gasstations,Client,Stations,Metrics,Arrays,Microsoft,Array,Html,Default,List,T,IntrinsicFunctionProxy,OperatorIntrinsics,jQuery,Pervasives,Strings,Math,Bing,Maps,Rest;
 Runtime.Define(Global,{
  websharper:{
   gasstations:{
    Client:{
     AddStations:function(max,map)
     {
      return Concurrency.Start(Concurrency.Delay(function()
      {
       ...
       {
        var loc,radius,x1,locs;
        loc=map.getCenter();
        radius=Metrics.Distance(loc,map.getBounds().getNorthwest());
        x1=Arrays.choose(function(s)
        {
         var d;
         d=Metrics.Distance(loc,new Microsoft.Maps.Location(s.latitude,s.longitude));
         return d<=radius?{
          $:1,
          $0:[s,d]
         }:{
     ...
     Metrics:{
      ...
      {
       var toRad,lat1,lat2,dlat,dlong,a1;
       toRad=function(x)
       {
       ...
       dlong=toRad(b.longitude-a.longitude);
       a1=Math.sin(dlat/2)*Math.sin(dlat/2)+Math.cos(lat1)*Math.cos(lat2)*Math.sin(dlong/2)*Math.sin(dlong/2);
       return 6371*(2*Math.atan2(Math.sqrt(a1),Math.sqrt(1-a1)));
      }
     },
     ...
     {
      map.getCenter();
      return Client.AddStations(150,map);
     },
 ...
 Runtime.OnInit(function()
 {
  ...
  Metrics=Runtime.Safe(Client.Metrics);
  Arrays=Runtime.Safe(WebSharper.Arrays);

...
(function()
{
 var Global=this,Runtime=this.IntelliFactory.Runtime,WebSharper,Unchecked,Arrays,Bing,Maps,Rest,Strings,document,Html,Client,Default,List,Seq;
 Runtime.Define(Global,{
  IntelliFactory:{
   WebSharper:{
    Bing:{
     Maps:{
      Rest:{
       ...
       RequestImageryMetadata:function(credentials,request,callback)
       {
        ...
        if(!Rest.IsUndefined(request.centerPoint))
         {
          self=request.centerPoint;
          _="/"+(self.x+","+self.y);
         }
        else
         {
        ...
        return Rest.SendRequest(Rest.restApiUri()+"Imagery/Metadata/"+Global.String(request.imagerySet)+_+"?"+req+"&"+Rest.RequestStringBoilerplate(credentials));
       },
       RequestLocationByAddress:function(credentials,address,callback)
       {
        var req;
        Global[Rest.RequestCallbackName()]=callback;
        req=Strings.concat("&",Rest.OptionalFields(address,["adminDistrict","locality","addressLine","countryRegion","postalCode"]));
        return Rest.SendRequest(Rest.restApiUri()+"Locations?"+req+"&"+Rest.RequestStringBoilerplate(credentials));
       },
       RequestLocationByPoint:function(credentials,x,y,entities,callback)
       {
        Global[Rest.RequestCallbackName()]=callback;
        return Rest.SendRequest(Rest.restApiUri()+"Locations/"+Global.String(x)+","+Global.String(y)+"?"+Rest.RequestStringBoilerplate(credentials)+(entities.$==0?"":"&includeEntityTypes="+Strings.concat(",",entities)));
       },
       RequestLocationByQuery:function(credentials,query,callback)
       {
        Global[Rest.RequestCallbackName()]=callback;
        return Rest.SendRequest(Rest.restApiUri()+"Locations?query="+query+"&"+Rest.RequestStringBoilerplate(credentials));
       },
       RequestRoute:function(credentials,request,callback)
       {
        var req,array2;
        Global[Rest.RequestCallbackName()]=callback;
        array2=Rest.OptionalFields(request,["avoid","heading","optimize","routePathOutput","distanceUnit","dateTime","timeType","maxSolutions","travelMode"]);
        req=Strings.concat("&",Rest.StringifyWaypoints(request.waypoints).concat(array2));
        return Rest.SendRequest(Rest.restApiUri()+"/Routes?"+req+"&"+Rest.RequestStringBoilerplate(credentials));
       },
       RequestRouteFromMajorRoads:function(credentials,request,callback)
       {
        var req;
        Global[Rest.RequestCallbackName()]=callback;
        req=Strings.concat("&",Rest.OptionalFields(request,["destination","exclude","routePathOutput","distanceUnit"]));
        return Rest.SendRequest(Rest.restApiUri()+"/Routes/FromMajorRoads?"+req+"&"+Rest.RequestStringBoilerplate(credentials));
       },
       ...
       SendRequest:function(req)
       {
        ...
        _this=Default.Attr();
        _this1=Default.Attr();
        document.documentElement.appendChild(Default.Script(List.ofArray([_this.NewAttr("type","text/javascript"),_this1.NewAttr("src",req)])).Dom);
       },
       StaticMap:function(credentials,request)
       {
        var _this,x;
        _this=Default.Attr();
        x=Rest.StaticMapUrl(credentials,request);
        return Default.Img(List.ofArray([_this.NewAttr("src",x)]));
       },
       StaticMapUrl:function(credentials,request)
       {
        var fields,query,self2,hasRoute,req;
        fields=Seq.toList(Seq.delay(function()
        {
         ...
         {
          var _,self,self1;
          if(!Rest.IsUndefined(request.mapArea))
           {
            self=request.mapArea[0];
            self1=request.mapArea[1];
            _=[self.x+","+self.y+","+(self1.x+","+self1.y)];
           }
          else
           {
            _=Seq.empty();
          ...
          return Seq.append(_,Seq.delay(function()
          {
           return Seq.append(!Rest.IsUndefined(request.mapSize)?[Global.String(request.mapSize[0])+","+Global.String(request.mapSize[1])]:Seq.empty(),Seq.delay(function()
           {
            ...
            {
             return"pp="+(Global.String(pin.x)+","+Global.String(pin.y)+";"+(Rest.IsUndefined(pin.iconStyle)?"":Global.String(pin.iconStyle))+";"+(Rest.IsUndefined(pin.label)?"":pin.label));
            },request.pushpin):Seq.empty(),Seq.delay(function()
            {
             return Seq.append(!Rest.IsUndefined(request.waypoints)?Rest.StringifyWaypoints(request.waypoints):Seq.empty(),Seq.delay(function()
             {
              ...
              {
               return!Rest.IsUndefined(request.distanceBeforeFirstTurn)?["dbft="+Global.String(request.distanceBeforeFirstTurn)]:Seq.empty();
              }));
             }));
            }));
           }));
        ...
        if(!Rest.IsUndefined(request.query))
         {
          query=request.query;
         }
        else
         {
          if(!Rest.IsUndefined(request.centerPoint))
           {
            self2=request.centerPoint;
            query=self2.x+","+self2.y+"/"+Global.String(request.zoomLevel);
           }
          else
           {
            query="";
 ...
 Runtime.OnInit(function()
 {
  ...
  Maps=Runtime.Safe(Bing.Maps);
  Rest=Runtime.Safe(Maps.Rest);
  Strings=Runtime.Safe(WebSharper.Strings);
  document=Runtime.Safe(Global.document);
  Html=Runtime.Safe(WebSharper.Html);
  Client=Runtime.Safe(Html.Client);
...
(function()
{
 var Global=this,Runtime=this.IntelliFactory.Runtime,websharper,gasstations,Client,Stations,Metric,WebSharper,Arrays,Array,Microsoft,Concurrency,IntrinsicFunctionProxy,OperatorIntrinsics,jQuery,JavaScript,Pervasives,List,Strings,Math,Html,Client1,Default,Bing,Maps,Rest;
 Runtime.Define(Global,{
  websharper:{
   gasstations:{
    Client:{
     AddStations:function(max,map)
     {
      var f,arg00,t;
      f=function()
      {
       ...
       f1=function(_arg1)
       {
        var a,loc,radius,x1,locs;
        a=map.getCenter();
        loc=Metric.locationToLoc(a);
        radius=Metric.Distance(loc,Metric.locationToLoc(map.getBounds().getNorthwest()));
        x1=Arrays.filter(function(s)
        {
         return Metric.Distance(loc,Metric.mkLoc(s.latitude,s.longitude))<=radius;
        },_arg1);
        locs=Array.take(max,x1);
        Arrays.iter(function(s)
        {
         var loc1,pin,box;
         loc1=new Microsoft.Maps.Location(s.latitude,s.longitude);
         pin=new Microsoft.Maps.Pushpin(loc1);
         box=new Microsoft.Maps.Infobox(loc1,{
          description:"<h3>"+s.station_name+"</h3>"+"<p>"+s.street_address+"</p>",
          visible:false
         });
     ...
     AddressCallback:function(map,response)
     {
      var coords;
      coords=IntrinsicFunctionProxy.GetArray(IntrinsicFunctionProxy.GetArray(response.resourceSets,0).resources,0).point.coordinates;
      return map.setView({
       center:new Microsoft.Maps.Location(IntrinsicFunctionProxy.GetArray(coords,0),IntrinsicFunctionProxy.GetArray(coords,1)),
       zoom:12
      });
     },
     Array:{
      take:function(n,arr)
      {
       return n<=IntrinsicFunctionProxy.GetLength(arr)?OperatorIntrinsics.GetArraySlice(arr,{
        $:1,
        $0:0
       },{
        $:1,
        $0:n-1
       }):OperatorIntrinsics.GetArraySlice(arr,{
        $:1,
        $0:0
       },{
        $:1,
        $0:IntrinsicFunctionProxy.GetLength(arr)-1
       });
      }
     },
     InfoClick:function(e)
     {
      return e.target.setOptions({
       visible:false
      });
     },
     Main:Runtime.Field(function()
     {
      var _MainApp_213_1,_app_219_1;
      Global.ko.bindingHandlers.element={
       update:Runtime.Tupled(function(tupledArg)
       {
        var element,x;
        element=tupledArg[0];
        x=Global.ko.unwrap(tupledArg[1].call(null,null));
        jQuery(element).empty();
        return jQuery(element).append(x);
       })
      };
      _MainApp_213_1={};
      _MainApp_213_1.app=new Global.DevExpress.framework.html.HtmlApplication(Pervasives.NewFromList(List.ofArray([["namespace",_MainApp_213_1],["navigationType","simple"]])));
      _app_219_1=_MainApp_213_1.app;
      _MainApp_213_1.Map=function()
      {
       var searchQuery,patternInput,map,viewModel;
       searchQuery=Global.ko.observable().extend({
        throttle:500
       });
       patternInput=Client.MkMap();
       map=patternInput[1];
       viewModel={
        Map:patternInput[0].Dom,
        SearchQuery:searchQuery,
        LoadPanel:{
         Visible:Global.ko.computed(function()
         {
       ...
       searchQuery.subscribe(function(value)
       {
        ...
        return!Strings.IsNullOrEmpty(s)?Client.SearchAddress(map,value):null;
       });
       return viewModel;
      };
      _app_219_1.router.register(":view",{
       view:"Map"
      });
      return _app_219_1.navigate();
     }),
     Metric:{
      Distance:function(a,b)
      {
       var lat1,lat2,dlat,dlong,sinlat,sinlong,a1,c;
       lat1=Metric.toRad(a.lat);
       lat2=Metric.toRad(b.lat);
       dlat=Metric.toRad(b.lat-a.lat);
       dlong=Metric.toRad(b["long"]-a["long"]);
       sinlat=Math.sin(dlat/2);
       sinlong=Math.sin(dlong/2);
       a1=sinlat*sinlat+Math.cos(lat1)*Math.cos(lat2)*sinlong*sinlong;
      ...
      locationToLoc:function(a)
      {
       return Metric.mkLoc(a.latitude,a.longitude);
      },
      mkLoc:function(lat,_long)
      {
       return{
        lat:lat,
        "long":_long
     ...
     MkMap:function()
     {
      var x,options,view,map;
      x=Default.Div(List.ofArray([Default.Attr().Class("dx-map-container")]));
      options={
       credentials:Client.bingMapsKey()
      };
      view={
       center:new Microsoft.Maps.Location(40.806862,-96.681679),
       zoom:5
      };
      map=new Microsoft.Maps.Map(x.get_Body(),options);
      map.setView(view);
      map.setMapType(Microsoft.Maps.MapTypeId.road);
      Microsoft.Maps.Events.addHandler(map,"viewchangeend",function(_arg10_)
      {
       return Client.TracingLocation(map,_arg10_);
      });
      return[x,map];
     },
     PinClick:function(box)
     {
      return box.setOptions({
       visible:true
      });
     },
     SearchAddress:function(map,address)
     {
      var addr;
      addr={
       locality:address,
      ...
      return Rest.RequestLocationByAddress(Client.bingMapsKey(),addr,function(response)
      {
       return Client.AddressCallback(map,response);
      });
     },
     Stations:{
      ...
      {
       var url,callback;
       url="https://developer.nrel.gov/api/alt-fuel-stations/v1.json?api_key="+Stations.nrelKey();
       callback=Runtime.Tupled(function(tupledArg)
       {
        var ok;
        ok=tupledArg[0];
        ...
        {
         var _;
         _=data.fuel_stations;
         Stations.Cache=function()
         {
          return _;
         };
         (Stations.fetched())(true);
         ok(Stations.Cache());
         return true;
        });
        return;
      ...
      fetched:Runtime.Field(function()
      {
       return Global.ko.observable(false);
      }),
      nrelKey:Runtime.Field(function()
      {
       return"PMwnNRqcydLTX3KHJBXC31Lf2H2mZaGesNPQsNzr";
      })
     },
     TracingLocation:function(map)
     {
      map.getCenter();
      return Client.AddStations(75,map);
     },
 ...
 Runtime.OnInit(function()
 {
  websharper=Runtime.Safe(Global.websharper);
  gasstations=Runtime.Safe(websharper.gasstations);
  Client=Runtime.Safe(gasstations.Client);
  Stations=Runtime.Safe(Client.Stations);
  Metric=Runtime.Safe(Client.Metric);
  WebSharper=Runtime.Safe(Global.IntelliFactory.WebSharper);
  Arrays=Runtime.Safe(WebSharper.Arrays);
  Array=Runtime.Safe(Client.Array);
  ...
  Bing=Runtime.Safe(WebSharper.Bing);
  Maps=Runtime.Safe(Bing.Maps);
  return Rest=Runtime.Safe(Maps.Rest);
 });
 Runtime.OnLoad(function()
 {
  Client.bingMapsKey();
